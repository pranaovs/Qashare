...
func RegisterGroupsRoutes(router *gin.RouterGroup, pool *pgxpool.Pool) {
	...
	router.GET("/:id/expenses", middleware.RequireAuth(), middleware.RequireGroupMember(pool), handler.ListGroupExpenses)
	router.GET("/:id/settlements", middleware.RequireAuth(), middleware.RequireGroupMember(pool), handler.GetSettlements)
}
